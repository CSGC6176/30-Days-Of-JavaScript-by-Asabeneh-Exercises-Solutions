...
let sort_name=countries_data.sort((a,b)=>{
...
// let mostSpokenLanguages=countries_data.sort((a,b)=>{
...
// let mostSpokenLanguages = function(arr){
//   let fd=arr.reduce((acc,item)=>{
...
let mostPopulatedCountries=(arr,n)=>{
  ...
  let result=data.map(a=>{
        return `{ country: ${a.name},  population: ${a.population}}`
  })
  return result
}
...
const statistics={
  ...
  range:function(){
    return Math.abs(Math.max(...ages)-Math.min(...ages))
  },
  mean:function(){
    return Math.ceil(statistics.sum()/ages.length)
  },
  median:function(){
    let sort=ages.sort(function(a,b){return a-b})
  ...
  mode:function(){
    let sortedArr=ages.sort(function(a,b){return a-b})
    const count={}
    for(let i=0;i<sortedArr.length;i++){
      count[sortedArr[i]]=(count[sortedArr[i]] || 0)+1
...
var:function(){
   let variance=ages.map((value)=>{
       return Math.pow(value-statistics.mean(),2);
    }).reduce((acc,curr)=>acc+curr,0)/ages.length
return variance  },

std:function(){
 return Math.sqrt(statistics.var()).toFixed(1)
},

freqDist:function(){
  let fd=ages.reduce((acc,item)=>{
    acc[item]=(acc[item]||0)+1
    return acc
  },{})
  return fd
